...
## Also on Windows, and on your phone

The same product, on the two platforms the Mac app does not reach. Both run one shared web core
(`web/`), so they are one codebase rather than two ports: the same hold-and-tap state machine, the
same history, the same pre-roll and tail. `web/src/bridge.js` is the only file that knows which one
it is inside.

`Sources/` is untouched by any of it.

**Windows** is a WebView2 shell around that core, downloaded as one EXE. On first run it offers to
install itself into `%LOCALAPPDATA%\Programs\Tiro`, with a Start menu entry and an Apps and
Features entry, and never asks for administrator rights. The native layer is the global hotkey,
`SendInput`, DPAPI for the key, the tray and the pill; everything else is the web core.

<div align="center">
<img src="docs/windows/install-prompt.png" width="420" alt="Tiro's Windows install prompt, offering to install, run once, or stay portable">
<img src="docs/windows/app-window.png" width="240" alt="The Tiro window on Windows">
</div>

**The phone** is a PWA, and one thing about it has to be said before anything else: **it cannot type
into other apps on iOS.** No global hotkey, no background microphone, no API for inserting text into
another app's field. So it is scoped as a fast dictation scratchpad instead: hold, speak, and the
transcript is on your clipboard before you switch apps. Installing it to the Home Screen matters
more than it looks, because Safari can clear a tab's `localStorage` after seven days and that is
where the key lives.

<div align="center">
<img src="docs/web/phone-setup.png" width="200" alt="Tiro on a phone, asking for a Deepgram key">
<img src="docs/web/phone-recording.png" width="200" alt="Tiro on a phone, recording">
<img src="docs/web/phone-take.png" width="200" alt="A finished transcript on a phone">
</div>

More screenshots are in [`docs/web/`](docs/web) and [`docs/windows/`](docs/windows).
